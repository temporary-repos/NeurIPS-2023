...
class Transition_Recurrent(nn.Module):
    ...
    def __init__(self, z_dim, transition_dim, identity_init=True, domain=False, stochastic=True):
        ...
        if domain:
            ...
            # compute the proposed_nonstationary mean
...
class DKF(LatentMetaDynamicsModel):
    def __init__(self, args, top, exptop):
        ...
        # domain
        self.domain_function = LatentDomainEncoderDKF(self.args.num_filt, 1, self.args.latent_dim, stochastic=False)
    ...
    def latent_dynamics(self, T, x_rnn, z_c):
        ...
        for t in range(self.args.z_amort):
            ...
            mu_q, var_q = self.estimation(x_rnn[:, t, :], z_prev, rnn_bidirection=True, z_c=z_c[:, t])
    ...
    def forward(self, x, domain, labels, generation_len):
        ...
        K = domain.shape[1]
        z_c = self.latent_domain(domain, K)
